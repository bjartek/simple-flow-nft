...
access(all) contract BasicNFT : SimpleNFT{
    ...
    access(all) resource NFT: SimpleNFT.DisplayableNFT, NonFungibleToken.NFT{
        ...
        access(all) fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<MetadataViews.NFTCollectionData>(): return BasicNFT.getCollectionData()
                case Type<MetadataViews.NFTCollectionDisplay>(): return BasicNFT.getCollectionDisplay()
            }
            return nil
        }
    ...
    access(all) resource Minter {
        access(all) fun mintNFT(metadata: {String: AnyStruct}, receiver : &{NonFungibleToken.Receiver}){
            ...
            emit Minted(id: nft.id, uuid:nft.uuid, to: receiver.owner?.address, type: Type<@BasicNFT.NFT>().identifier)
